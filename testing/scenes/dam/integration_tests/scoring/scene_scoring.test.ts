...
describe('Scoring - Dam Scene', () => {
  ...
  describe('First Visit Scoring', () => {
    it('should award points for first visit to dam', () => {
      // Reset scoring state
      scoringHelper.resetScoringState();
      testEnv.services.gameState.setCurrentScene('dam');
      testEnv.services.gameState.updateSceneRuntimeState('dam', { visited: false });
      ...
      expect(testEnv.services.gameState.hasVisitedScene('dam')).toBe(true);
