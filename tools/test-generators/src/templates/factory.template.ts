 ...
 * Integration Test Factory for {{title}}
 * Auto-generated factory for {{id}} scene tests
 */
...
import { {{className}}Helper } from './{{id}}_helper';
...
export class {{factoryName}} {
  /**
   * Create a complete integration test environment for {{title}}
   */
  static async createTestEnvironment(): Promise<{{testEnvType}}> {
    ...
    if (!services.combat) {
      services.combat = {{factoryName}}.createMockCombatService() as any;
    }
    if (!services.persistence) {
      services.persistence = {{factoryName}}.createMockPersistenceService() as any;
    }
    ...
    // Set player to {{id}} scene
    services.gameState.setCurrentScene('{{id}}');

    // Create helpers
    const {{helperName}} = new {{className}}Helper(
