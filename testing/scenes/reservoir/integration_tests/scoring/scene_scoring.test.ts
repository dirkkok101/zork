...
describe('Scoring - Reservoir Scene', () => {
  ...
  describe('Treasure Collection Scoring', () => {
    it('should NOT award points when taking trunk of jewels (points only on deposit)', () => {
      // Setup: Reset scoring and ensure treasure is in scene
      scoringHelper.resetScoringState();
      testEnv.services.scene.addItemToScene('reservoir', 'trunk');

      const initialScore = scoringHelper.getCurrentScore();

      // Execute: Take treasure for first time
      const result = testEnv.commandProcessor.processCommand('take trunk of jewels');
      ...
      if (result.success) {
        ...
        expect(scoringHelper.isTreasureFound('trunk')).toBe(true); // But treasure is marked as found
      }
    });
    ...
    it('should maintain treasure found flag even without points', () => {
      ...
      if (result.success) {
        ...
        expect(scoringHelper.isTreasureFound('trunk')).toBe(true);
        scoringHelper.verifyNoScoreChange(result);
      }
    });
  ...
  describe('Scoring State Integrity', () => {
    ...
    it('should not award negative scores', () => {
      ...
      testEnv.commandProcessor.processCommand('look');
      testEnv.services.scene.addItemToScene('', 'trunk');
    ...
    it('should maintain treasure found flags correctly', () => {
      ...
      expect(scoringHelper.isTreasureFound('trunk')).toBe(false);
