 ...
 * Auto-generated factory for forest_4 scene tests
 */
...
import { Forest4Helper } from './forest_4_helper';
import { LookCommandHelper } from '@testing/helpers/LookCommandHelper';
import log from 'loglevel';

export interface Forest4TestEnvironment {
  services: Services;
  commandProcessor: CommandProcessor;
  forest4Helper: Forest4Helper;
  lookCommandHelper: LookCommandHelper;
  cleanup: () => void;
  resetScoring: () => void;
}

export class Forest4IntegrationTestFactory {
  /**
   * Create a complete integration test environment for Forest
   */
  static async createTestEnvironment(): Promise<Forest4TestEnvironment> {
    ...
    if (!services.combat) {
      services.combat = Forest4IntegrationTestFactory.createMockCombatService() as any;
    }
    if (!services.persistence) {
      services.persistence = Forest4IntegrationTestFactory.createMockPersistenceService() as any;
    }
    ...
    // Set player to forest_4 scene
    services.gameState.setCurrentScene('forest_4');

    // Create helpers
    const forest4Helper = new Forest4Helper(
