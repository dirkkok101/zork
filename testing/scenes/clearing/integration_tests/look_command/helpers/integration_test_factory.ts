/**
 * Integration Test Factory for Clearing
 * Auto-generated factory for clearing scene tests
 */
...
import { ClearingHelper } from './clearing_helper';
import { LookCommandHelper } from '@testing/helpers/LookCommandHelper';
import log from 'loglevel';

export interface ClearingTestEnvironment {
  services: Services;
...
export class ClearingIntegrationTestFactory {
  /**
   * Create a complete integration test environment for Clearing
   */
  static async createTestEnvironment(): Promise<ClearingTestEnvironment> {
    ...
    if (!services.combat) {
      services.combat = ClearingIntegrationTestFactory.createMockCombatService() as any;
    }
    if (!services.persistence) {
      services.persistence = ClearingIntegrationTestFactory.createMockPersistenceService() as any;
    }
    ...
    // Set player to clearing scene
    services.gameState.setCurrentScene('clearing');

    // Create helpers
    const clearingHelper = new ClearingHelper(
