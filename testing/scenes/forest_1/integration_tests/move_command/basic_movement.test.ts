 ...
 * Auto-generated tests for movement functionality from forest_1
 */

import '../setup';
import { Forest1TestEnvironment, Forest1IntegrationTestFactory } from '../look_command/helpers/integration_test_factory';
import { MoveCommandHelper } from '@testing/helpers/MoveCommandHelper';

describe('Move Command - Forest Scene', () => {
  let testEnv: Forest1TestEnvironment;
  let moveHelper: MoveCommandHelper;

  beforeEach(async () => {
    testEnv = await Forest1IntegrationTestFactory.createTestEnvironment();
  ...
  describe('Simple Movement', () => {
    it('should move north to forest_1', async () => {
    ...
    it('should move east to forest_3', async () => {
  ...
  describe('Scene Consistency', () => {
    it('should remain in forest_1 after invalid movement', async () => {
    ...
    it('should properly update scene after successful movement', async () => {
      ...
      expect(moveHelper.hasVisitedScene('forest_1')).toBe(true);
    });
  });
});
