/**
 * Integration Test Factory for South of House
 * Auto-generated factory for south_of_house scene tests
 */
...
import { SouthOfHouseHelper } from './south_of_house_helper';
import { LookCommandHelper } from '@testing/helpers/LookCommandHelper';
import log from 'loglevel';

export interface SouthOfHouseTestEnvironment {
  services: Services;
  commandProcessor: CommandProcessor;
  southOfHouseHelper: SouthOfHouseHelper;
  lookCommandHelper: LookCommandHelper;
  cleanup: () => void;
  resetScoring: () => void;
}

export class SouthOfHouseIntegrationTestFactory {
  /**
   * Create a complete integration test environment for South of House
   */
  static async createTestEnvironment(): Promise<SouthOfHouseTestEnvironment> {
    ...
    if (!services.combat) {
      services.combat = SouthOfHouseIntegrationTestFactory.createMockCombatService() as any;
    }
    if (!services.persistence) {
      services.persistence = SouthOfHouseIntegrationTestFactory.createMockPersistenceService() as any;
    }
    ...
    // Set player to south_of_house scene
    services.gameState.setCurrentScene('south_of_house');

    // Create helpers
    const southOfHouseHelper = new SouthOfHouseHelper(
