...
describe('Scoring - Clearing Scene', () => {
  ...
  describe('First Visit Scoring', () => {
    it('should award points for first visit to clearing', () => {
      // Reset scoring state
      scoringHelper.resetScoringState();
      testEnv.services.gameState.setCurrentScene('clearing');
      testEnv.services.gameState.updateSceneRuntimeState('clearing', { visited: false });
      ...
      expect(testEnv.services.gameState.hasVisitedScene('clearing')).toBe(true);
  ...
  describe('Non-Treasure Items', () => {
    it('should not award points for taking non-treasure items like pile of leaves', () => {
      // Setup: Non-treasure in scene
      scoringHelper.resetScoringState();
      testEnv.services.scene.addItemToScene('clearing', 'leave');

      const initialScore = scoringHelper.getCurrentScore();

      // Execute: Take non-treasure
      const result = testEnv.commandProcessor.processCommand('take pile of leaves');

      if (result.success) {
        // Verify: No scoring for non-treasure
        expect(scoringHelper.isTreasure('leave')).toBe(false);
        scoringHelper.verifyNoScoreChange(result);
