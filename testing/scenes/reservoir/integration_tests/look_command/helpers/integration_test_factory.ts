/**
 * Integration Test Factory for Reservoir
 * Auto-generated factory for reservoir scene tests
 */
...
import { ReservoirHelper } from './reservoir_helper';
import { LookCommandHelper } from '@testing/helpers/LookCommandHelper';
import log from 'loglevel';

export interface ReservoirTestEnvironment {
  services: Services;
  commandProcessor: CommandProcessor;
  reservoirHelper: ReservoirHelper;
  lookCommandHelper: LookCommandHelper;
  cleanup: () => void;
  resetScoring: () => void;
}

export class ReservoirIntegrationTestFactory {
  /**
   * Create a complete integration test environment for Reservoir
   */
  static async createTestEnvironment(): Promise<ReservoirTestEnvironment> {
    ...
    if (!services.combat) {
      services.combat = ReservoirIntegrationTestFactory.createMockCombatService() as any;
    }
    if (!services.persistence) {
      services.persistence = ReservoirIntegrationTestFactory.createMockPersistenceService() as any;
    }
    ...
    // Set player to reservoir scene
    services.gameState.setCurrentScene('reservoir');

    // Create helpers
    const reservoirHelper = new ReservoirHelper(
