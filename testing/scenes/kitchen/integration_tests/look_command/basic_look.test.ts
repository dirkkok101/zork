 ...
 * Auto-generated tests for look functionality in Kitchen
 */

import '../setup';
import { KitchenTestEnvironment, KitchenIntegrationTestFactory } from './helpers/integration_test_factory';

describe('Basic Look Command - Kitchen Scene', () => {
  ...
  describe('Exit Display', () => {
    ...
    it('should verify expected exits are actually available', () => {
      testEnv.kitchenHelper.verifyExpectedExits();
    });
  });
  ...
  describe('Command Variations', () => {
    ...
    it('should produce same result for all basic look variations', async () => {
      ...
      expect(lookResult.message).toContain('Kitchen');
      expect(lResult.message).toContain('Kitchen');
      expect(lookAroundResult.message).toContain('Kitchen');
    });
  });
});
