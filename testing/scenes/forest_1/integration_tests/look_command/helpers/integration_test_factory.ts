 ...
 * Auto-generated factory for forest_1 scene tests
 */
...
import { Forest1Helper } from './forest_1_helper';
import { LookCommandHelper } from '@testing/helpers/LookCommandHelper';
import log from 'loglevel';

export interface Forest1TestEnvironment {
  services: Services;
  commandProcessor: CommandProcessor;
  forest1Helper: Forest1Helper;
  lookCommandHelper: LookCommandHelper;
  cleanup: () => void;
  resetScoring: () => void;
}

export class Forest1IntegrationTestFactory {
  /**
   * Create a complete integration test environment for Forest
   */
  static async createTestEnvironment(): Promise<Forest1TestEnvironment> {
    ...
    if (!services.combat) {
      services.combat = Forest1IntegrationTestFactory.createMockCombatService() as any;
    }
    if (!services.persistence) {
      services.persistence = Forest1IntegrationTestFactory.createMockPersistenceService() as any;
    }
    ...
    // Set player to forest_1 scene
    services.gameState.setCurrentScene('forest_1');

    // Create helpers
    const forest1Helper = new Forest1Helper(
