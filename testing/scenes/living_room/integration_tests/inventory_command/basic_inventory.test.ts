/**
 * Inventory Command Tests - Living Room Scene
...
describe('Inventory Command - Living Room Scene', () => {
  ...
  describe('Single Item in Inventory', () => {
    it('should display single item when carrying one item', () => {
      ...
      inventoryHelper.clearInventory();
      inventoryHelper.addItemToInventory('lamp');

      const result = inventoryHelper.executeInventoryDisplay();

      inventoryHelper.verifySuccess(result);
      expect(result.message).toMatch(/carrying|have/i);
      expect(result.message.toLowerCase()).toContain('lamp'.toLowerCase());
      inventoryHelper.verifyNoMove(result);
    });
  });
  ...
  describe('Multiple Items in Inventory', () => {
    it('should display multiple items when carrying several items', () => {
      ...
      inventoryHelper.clearInventory();
      inventoryHelper.addItemToInventory('lamp');
      inventoryHelper.addItemToInventory('rug');
      inventoryHelper.addItemToInventory('paper');
      ...
      expect(result.message.toLowerCase()).toContain('lamp'.toLowerCase());
      expect(result.message.toLowerCase()).toContain('carpet'.toLowerCase());
      expect(result.message.toLowerCase()).toContain('newspaper'.toLowerCase());
    ...
    it('should update inventory display when items are added', () => {
      ...
      expect(oneItemResult.message.toLowerCase()).toContain('lamp'.toLowerCase());

      // Add second item
      inventoryHelper.addItemToInventory('rug');

      const twoItemsResult = inventoryHelper.executeInventoryDisplay();
      expect(twoItemsResult.message.toLowerCase()).toContain('lamp'.toLowerCase());
      expect(twoItemsResult.message.toLowerCase()).toContain('carpet'.toLowerCase());
    });
    ...
    it('should update inventory display when items are removed', () => {
      ...
      // Remove one item
      inventoryHelper.removeItemFromInventory('lamp');

      const oneItemResult = inventoryHelper.executeInventoryDisplay();
      expect(oneItemResult.message.toLowerCase()).toContain('carpet'.toLowerCase());
      expect(oneItemResult.message.toLowerCase()).not.toContain('lamp'.toLowerCase());
    });
  });
  ...
  describe('Game State Tracking', () => {
    ...
    it('should track inventory changes across commands', () => {
      ...
      expect(updatedResult.message.toLowerCase()).toContain('lamp'.toLowerCase());
