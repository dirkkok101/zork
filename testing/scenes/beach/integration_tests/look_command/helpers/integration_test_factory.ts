/**
 * Integration Test Factory for Sandy Beach
 * Auto-generated factory for beach scene tests
 */
...
import { BeachHelper } from './beach_helper';
import { LookCommandHelper } from '@testing/helpers/LookCommandHelper';
import log from 'loglevel';

export interface BeachTestEnvironment {
  services: Services;
  commandProcessor: CommandProcessor;
  beachHelper: BeachHelper;
  lookCommandHelper: LookCommandHelper;
  cleanup: () => void;
  resetScoring: () => void;
}

export class BeachIntegrationTestFactory {
  /**
   * Create a complete integration test environment for Sandy Beach
   */
  static async createTestEnvironment(): Promise<BeachTestEnvironment> {
    ...
    if (!services.combat) {
      services.combat = BeachIntegrationTestFactory.createMockCombatService() as any;
    }
    if (!services.persistence) {
      services.persistence = BeachIntegrationTestFactory.createMockPersistenceService() as any;
    }
    ...
    // Set player to beach scene
    services.gameState.setCurrentScene('beach');

    // Create helpers
    const beachHelper = new BeachHelper(
