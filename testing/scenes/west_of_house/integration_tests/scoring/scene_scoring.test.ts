/**
 * Scoring Tests - West of House Scene
...
describe('Scoring - West of House Scene', () => {
  ...
  describe('First Visit Scoring', () => {
    it('should award points for first visit to west_of_house', () => {
      // Reset scoring state
      scoringHelper.resetScoringState();
      testEnv.services.gameState.setCurrentScene('west_of_house');
      testEnv.services.gameState.updateSceneRuntimeState('west_of_house', { visited: false });
      ...
      expect(testEnv.services.gameState.hasVisitedScene('west_of_house')).toBe(true);
  ...
  describe('Non-Treasure Items', () => {
    it('should not award points for taking non-treasure items like welcome mat', () => {
      // Setup: Non-treasure in scene
      scoringHelper.resetScoringState();
      testEnv.services.scene.addItemToScene('west_of_house', 'mat');

      const initialScore = scoringHelper.getCurrentScore();

      // Execute: Take non-treasure
      const result = testEnv.commandProcessor.processCommand('take welcome mat');

      if (result.success) {
        // Verify: No scoring for non-treasure
        expect(scoringHelper.isTreasure('mat')).toBe(false);
        scoringHelper.verifyNoScoreChange(result);
